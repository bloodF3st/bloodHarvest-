...
Настройка через переменную `MALLOC_CONF` в `.env` или в `[Service]` секции systemd:
...
**На практике:** бот обработал нагрузку, RSS вырос до 80 МБ → через 10 секунд простоя jemalloc отдаёт лишнее обратно → RSS падает до ~30 МБ. С glibc эти 80 МБ висели бы постоянно.